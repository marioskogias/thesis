...
the CTF event trace information formated as a Python object which has some
properties like the event name and the timestamp and another generator that
returns the event information, namely the values passed to the
\texttt{tracepoint} function call.  After that we can manipulate the data the
way we want.

The case of the JSON format is easy since after that we 
